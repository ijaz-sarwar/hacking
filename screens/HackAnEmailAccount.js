...
import MobilePhoneHack_Button from '../components/HackAnEmailAccount_Button';
...
Processing triggers for man-db (2.8.7-3) ...{"\n"}  
          ...
          else if(text.includes("sudo apt-get install hydra")) {
            children = <Text style={styles.txxt}>
            getting packages ... {"\n"}  
            ...
            succesfully installed ... {"\n"}  
              ...
              else if(text.includes("run hydra ")) {
                children = <Text style={styles.txxt}>
                running hydra ... {"\n"}  
                making the scripts ready ... {"\n"}  
                shaking the terminal ... {"\n"}  
                hydra ready to work ... {"\n"}  
                scripts are ready to deploy ...{"\n"}  
                type in the mail you want to hack : 
                  </Text>
                  }       
                  
                  else if(text.includes("user12@gmail.com")) {
                    children = <Text style={styles.txxt}>
                  bruteforcing the list ... {"\n"}  
                  bruteforcing tries on the server ... {"\n"}  
                succesfully bruteforced the email passwords {"\n"}  
                the password is :"G/8 ISlAMMABAD"{"\n"}  
                you have succesfully hacked it  :))))
                   </Text>
                  this.adds_show()
                      }       
